defmodule NeoSaxyTest do
  use NeoSaxyTest.ParsingCase, async: true

  alias NeoSaxyTest.StackHandler
  ...
  test "parses file streams" do
    for fixture_name <- @fixtures do
      ...
      assert {:ok, _state} = NeoSaxy.parse_stream(stream, StackHandler, [])
    end
  end
  ...
  test "maps file streams" do
    for fixture <- @fixtures do
      ...
      element_stream = NeoSaxy.stream_events(stream)
      assert [_ | _] = Enum.to_list(element_stream)
    end

    assert_raise NeoSaxy.ParseError, fn ->
      Enum.to_list(NeoSaxy.stream_events(stream_fixture("incorrect.xml")))
    end
  end
  ...
  test "handles trailing Unicode codepoints during streaming" do
    ...
    assert {:ok, event_stack} = NeoSaxy.parse_stream(stream, StackHandler, [])
  ...
  test "parse_stream/4 emits characters when they reach the max length limit" do
    ...
    assert {:ok, state} = NeoSaxy.parse_stream(doc, StackHandler, [], character_data_max_length: character_data_max_length)
    ...
    assert {:ok, state} = NeoSaxy.parse_stream(doc, StackHandler, [])
  ...
  describe "encode!/2" do
    ...
    test "encodes XML document into string" do
      ...
      assert NeoSaxy.encode!(root, version: "1.0") == ~s(<?xml version="1.0"?><foo>foo</foo>)
    end
  end
  ...
  describe "encode_to_iodata!/2" do
    ...
    test "encodes XML document into IO data" do
      root = element("foo", [], "foo")
      assert xml = NeoSaxy.encode_to_iodata!(root, version: "1.0")
      assert is_list(xml)
