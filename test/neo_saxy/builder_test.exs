defmodule NeoSaxy.BuilderTest do
  use ExUnit.Case, async: true
  use ExUnitProperties

  import NeoSaxy.Builder, only: [build: 1]

  doctest NeoSaxy.Builder

  test "builds pre-built simple-form element" do
    element = NeoSaxy.XML.element(:foo, [], [])
    assert build(element) == element

    element = NeoSaxy.XML.empty_element(:foo, [])
    assert build(element) == element

    characters = NeoSaxy.XML.characters("foo")
    assert build(characters) == characters

    cdata = NeoSaxy.XML.cdata("foo")
    assert build(cdata) == cdata

    reference = NeoSaxy.XML.reference(:entity, "foo")
    assert build(reference) == reference

    comment = NeoSaxy.XML.comment("foo")
  ...
  defmodule Struct do
    @derive {NeoSaxy.Builder, name: :test, attributes: [:foo], children: [:bar]}
  ...
  defmodule Category do
    @derive {NeoSaxy.Builder, name: :category, attributes: [:name]}
