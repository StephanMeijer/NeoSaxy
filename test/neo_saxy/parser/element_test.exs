defmodule NeoSaxy.Parser.ElementTest do
  use NeoSaxyTest.ParsingCase, async: true

  alias NeoSaxy.TestHandlers.StackHandler
  ...
  defp refute_parse(data, options \\ []) do
    assert {:error, error} = NeoSaxy.parse_string(data, StackHandler, [], options)

    stream = for <<char <- data>>, do: <<char>>
    assert {:error, _} = NeoSaxy.parse_stream(stream, StackHandler, [], options)

    error
  end

  defp assert_parse(data, options \\ []) do
    assert {:ok, events} = NeoSaxy.parse_string(data, StackHandler, [], options)

    stream = for <<char <- data>>, do: <<char>>
    assert NeoSaxy.parse_stream(stream, StackHandler, [], options) == {:ok, events}
