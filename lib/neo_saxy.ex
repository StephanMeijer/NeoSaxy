defmodule NeoSaxy do
  @moduledoc ~S"""
  NeoSaxy is an XML SAX parser and encoder.

  NeoSaxy provides functions to parse XML file in both binary and streaming way in compliant
  with [Extensible Markup Language (XML) 1.0 (Fifth Edition)](https://www.w3.org/TR/xml/).

  NeoSaxy also offers DSL and API to build, compose and encode XML document.
  ...
  * `:end_document` - when the root element is closed.
  ...
  NeoSaxy **only** supports UTF-8 encoding. It also respects the encoding set in XML document prolog, which means
  that if the declared encoding is not UTF-8, the parser stops. Anyway, when there is no encoding declared,
  NeoSaxy defaults the encoding to UTF-8.

  ### Reference expansion

  NeoSaxy supports expanding character references and XML 1.0 predefined entity references, for example `&#65;`
  is expanded to `"A"`, `&#x26;` to `"&"`, and `&amp;` to `"&"`.

  NeoSaxy does not expand external entity references, but provides an option to specify how they should be handled.
  ...
  NeoSaxy does not support parsing DTD (Doctype Definition) and XSD schemas. When encountering DTD, the parser simply
  skips that.
  ...
  * `:cdata_as_characters` - `true` to emit CData events as `:characters`. Defaults to `true`.
  ...
  Use `NeoSaxy.XML` to build and compose XML simple form, then `NeoSaxy.encode!/2`
  ...
  This function takes XML binary, SAX event handler (see more at `NeoSaxy.Handler`) and an initial state as the input, it returns
  `{:ok, state}` if parsing is successful, otherwise `{:error, exception}`, where `exception` is a
  `NeoSaxy.ParseError` struct which can be converted into readable message with `Exception.message/1`.
  ...
  This function takes a stream, SAX event handler (see more at `NeoSaxy.Handler`) and an initial state as the input, it returns
  `{:ok, state}` if parsing is successful, otherwise `{:error, exception}`, where `exception` is a
  `NeoSaxy.ParseError` struct which can be converted into readable message with `Exception.message/1`.
  ...
  `NeoSaxy.parse_stream/3` takes a `File.Stream` or `Stream` as the input, so the amount of bytes to buffer in each
  chunk can be controlled by `File.stream!/3` API.

  During parsing, the actual memory used by NeoSaxy might be higher than the number configured for each chunk, since
  NeoSaxy holds in memory some parsed parts of the original binary to leverage Erlang sub-binary extracting. Anyway,
  NeoSaxy tries to free those up when it makes sense.
  ...
  * `:character_data_max_length` - tells the parser to emit the `:characters` event when its length exceeds the specified
  ...
  When any parsing error occurs, it raises a `NeoSaxy.ParseError` exception.
  ...
  `NeoSaxy.stream_events/2` takes a `File.Stream` or `Stream` as the input, so the amount of bytes to buffer in each
  chunk can be controlled by `File.stream!/3` API.
  ...
  * `:character_data_max_length` - tells the parser to emit the `:characters` event when its length exceeds the specified
  ...
  @spec encode!(root :: NeoSaxy.XML.element(), prolog :: NeoSaxy.Prolog.t() | Keyword.t() | nil) :: String.t()
  ...
  """
  @spec encode_to_iodata!(root :: NeoSaxy.XML.element(), prolog :: NeoSaxy.Prolog.t() | Keyword.t() | nil) :: iodata()
