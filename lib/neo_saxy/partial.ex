defmodule NeoSaxy.Partial do
  alias NeoSaxy.Parser
  ...
  ## Example
      ...
      iex> {:ok, partial} = NeoSaxy.Partial.new(StackHandler, [])
      iex> {:cont, partial} = NeoSaxy.Partial.parse(partial, "<foo>")
      iex> {:cont, partial} = NeoSaxy.Partial.parse(partial, "</foo>")
      iex> NeoSaxy.Partial.terminate(partial)
      {:ok,
       [
         end_document: {},
         end_element: "foo",
  ...
  @doc """
  Builds up a `NeoSaxy.Partial`, which can be used for later parsing.
  """
  ...
  @spec new(
          ...
          initial_state :: term(),
          options :: Keyword.t()
        ) :: {:ok, partial :: t()} | {:error, exception :: NeoSaxy.ParseError.t()}
  ...
  * `{:error, exception}` - The parsing process has erred.
  ...
  i.e.
  `NeoSaxy.Partial.parse(partial, binary, new_state) # coninue previous partial with a new state`
  ...
  * `{:error, exception}` - The parsing process has erred.
  ...
  @spec terminate(partial :: t()) :: {:ok, state :: term()} | {:error, exception :: NeoSaxy.ParseError.t()}
